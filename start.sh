...
if [ $(ps aux | grep $USER | grep node | grep -v grep | wc -l | tr -s "\n") -eq 0 ]; then
    ...
    if cd $SCRIPT_PATH ; then
        ...
        ./node_modules/forever/bin/forever --uid 'tagmazing' -m 5 --plain --no-colors --minUptime 1000 --spinSleepTime 10000 -p $(pwd) -l $LOG -o $OUT_LOG -e $ERR_LOG start server/app.js
    fi
fi
